...
local etcd3 = require "etcd.etcd3"
...
local function testsetx(etcd_cli)
	...
	res, err = etcd_cli:get("hello")
	skynet.error(string.format("key %s is %s, create_revision: %s, mod_revision: %s", "hello",
		util_table.tostring(res.body.kvs[1].value), res.body.kvs[1].create_revision, res.body.kvs[1].mod_revision))
