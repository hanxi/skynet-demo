...
// 整数key加前缀
#define INT_KEY_PREV "_i_"
#define INT_KEY_PREV_SIZE (3)
...
static void append_table(struct bson *bs, lua_State *L, const char *key, size_t sz, int depth, bool array);
...
append_one(struct bson *bs, lua_State *L, const char *key, size_t sz, int depth) {
    ...
    case LUA_TTABLE:
        append_table(bs, L, key, sz, depth+1, false);
        break;
    case LUA_TBOOLEAN:
...
pack_dict_data(lua_State *L, struct bson *b, int depth, int kt) {
    ...
    switch(kt) {
        case LUA_TNUMBER:
            ...
            char tmp_i_key[32] = INT_KEY_PREV;
            strncpy(&tmp_i_key[INT_KEY_PREV_SIZE], key, sz);
            append_one(b, L, tmp_i_key, sz + INT_KEY_PREV_SIZE, depth);
            lua_pop(L,2);
            break;
        case LUA_TSTRING:
            key = lua_tolstring(L,-2,&sz);
...
static void
append_table(struct bson *bs, lua_State *L, const char *key, size_t sz, int depth, bool array) {
    if (depth > MAX_DEPTH) {
...
pack_ordered_dict(lua_State *L, struct bson *b, int n, int depth) {
    ...
    for (i=0;i<n;i+=2) {
        ...
        int vt = lua_type(L,-1);
        if (vt == LUA_TTABLE) {
            append_table(b, L, key, sz, depth+1, true);
        } else {
            append_one(b, L, key, sz, depth);
        }
        lua_pop(L,1);
...
unpack_dict(lua_State *L, struct bson_reader *br, bool array) {
    ...
    for (;;) {
        ...
        if (array) {
            ...
            if (klen > INT_KEY_PREV_SIZE
                && 0 == strncmp(INT_KEY_PREV, key, INT_KEY_PREV_SIZE)) {
                int id = strtol(key + INT_KEY_PREV_SIZE, NULL, 10);
                lua_pushinteger(L,id);
            } else {
